* @ValidationCode : MjotMTAzMTI3MjQ3MTpDcDEyNTI6MTU5MzUwMjMwMjYxMDp1c2VyOi0xOi0xOjA6MDpmYWxzZTpOL0E6REVWXzIwMTcxMC4wOi0xOi0x
* @ValidationInfo : Timestamp         : 30 Jun 2020 13:31:42
...
SUBROUTINE GB.MBL.E.DEP.LIST(Y.DATA)
...
*
    IF Y.CUS.ID THEN
        SEL.CMD = 'SELECT ':FN.AA.ARR:' WITH CUSTOMER EQ ':Y.CUS.ID:' AND PRODUCT.LINE EQ DEPOSITS AND ARR.STATUS EQ CURRENT AND CO.CODE EQ ':Y.COMP
    END
    IF Y.CUS.ID EQ "" THEN
        SEL.CMD = 'SELECT ':FN.AA.ARR:' WITH PRODUCT.LINE EQ DEPOSITS AND ARR.STATUS EQ CURRENT AND CO.CODE EQ ':Y.COMP
    END
...
*--------------------------------------------------------------------------------------------------
        IF Y.AC.NO EQ Y.ACC.NO THEN
            Y.DATA<-1> = Y.ACC.NO:'*':Y.AA.CUS.ID:'*':Y.PRODUCT:'*':Y.CURRENCY:'*':Y.AMT:'*':Y.ACC.VALUE.DATE:'*':Y.AD.MATURITY.DATE:'*':Y.INTEREST.RATE
*                           1              2            3             4            5               6                  7                    8
        END
        IF Y.AC.NO EQ "" THEN
            Y.DATA<-1> = Y.ACC.NO:'*':Y.AA.CUS.ID:'*':Y.PRODUCT:'*':Y.CURRENCY:'*':Y.AMT:'*':Y.ACC.VALUE.DATE:'*':Y.AD.MATURITY.DATE:'*':Y.INTEREST.RATE
